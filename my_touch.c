...
int main(int argc, char **argv) {
    ...
    const int inode_position = create_entry(path, NULL, permissions, INODE_FILE);
    if (inode_position < 0) return print_error(inode_position < FAILURE ? inode_position : NOT_CREATED);
