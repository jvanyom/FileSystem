...
int main(int argc, char **argv) {
    if (argc != 3) return failure(SYNTAX, argv[0], "<dispositivo> <número de bloques>");

    const unsigned int total_blocks = strtol(argv[2], NULL, 10);
    if (errno == EINVAL) return failure(NAN, "número de bloques");

    const unsigned int total_inodes = total_blocks / 4;
    ...
    const signed int failed = mount(argv[1]) < 0 ||
                              ...
                              init_bitmap() < 0 ||
                              init_inodes() < 0 ||
                              reserve_inode(DIR_INODE, RWX) < 0 ||
                              umount() < 0;

    return failed ? failure(MOUNT, argv[1]) : EXIT_SUCCESS;
}